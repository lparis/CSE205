...
public class Expression 
{
	 ...
	 * Expression(String)
	 *
	 * pExprStr is a string representing an infix expression, such as
	 * "(1 + 2) * -3". This ctor uses the Tokenizer class to break the string
	 * into Token objects which are stored in the token queue instance variable.
	 *
	 * PSEUDOCODE: Create a new Queue<Token> object and pass it to setTokenQueue
	 * (this initializes mTokenQueue) Declare and create a Tokenizer object
	 * named tokenizer passing pExprStr to the ctor Declare a Token object named
	 * prevToken initialized to null -- Read the first token Declare a Token
	 * object named token and assign the return value from tokenizer.nextToken()
	 * to it -- Keep reading tokens until tokenizer.nextToken() returns null
	 * While token is not null Do -- Check for and handle the negation operator.
	 * If token instanceof SubOperator Then token = negationCheck(token, pToken)
	 * End if -- Add the token to the queue. Call getTokenQueue().enqueue(token)
	...
	public Expression(String pExprStr) 
	{
		...
		Queue<Token> object = new Queue<Token>();
		setTokenQueue(object);
		...
		//Read the first token Declare a Token object named token and assign the return value 
		//from tokenizer.nextToken() to it
		...
		while (token != null) 
		{
			//If token instanceof SubOperator Then token = negationCheck(token, prevToken)
	 ...
	 *
	 * PSEUDOCODE: Declare and create a Stack<Operator> object named
	 * operatorStack Declare and create a Stack<Operand> object named
	 ...
	 */
	private boolean keepEvaluating(Stack<Operator> pOperatorStack,
			Operator pOperator) {
		if (pOperatorStack.isEmpty())
			return false;
		else
			return pOperatorStack.peek().stackPrecedence() >= pOperator
					.precedence();
	}
	...
	private Token negationCheck(Token pToken, Token pPrevToken) {
		if (pPrevToken == null || pPrevToken instanceof BinaryOperator
				|| pPrevToken instanceof LeftParen) {
	 ...
	 *
	 * PSEUDOCODE: Declare and create Operand object named right = Call
	 * pOperandStack.pop() Declare and create Operator object named operator =
	 * Call pOperatorStack.pop() If operator instanceof UnaryOperator Then
	 * Typecast operator to UnaryOperator and call evaluate(right) on it Push
	 * the returned Operand from the above statement onto the operand stack Else
	 ...
	 * End If
	 */
	private void topEval(Stack<Operator> pOperatorStack,
		Stack<Operand> pOperandStack) {
		Operand right = pOperandStack.pop();
		Operator operator = pOperatorStack.pop();
		if(operator instanceof NegOperator){
			right =((NegOperator) operator).evaluate(right);
			pOperandStack.push(right);
		}
		 else {
			Operand left = pOperandStack.pop();
			left = ((BinaryOperator) operator).evaluate(left, right);
			pOperandStack.push(left);
		}
	}
