...
//
// DESCRIPTION
// Implements a generic Doubly Linked List data structure.
//
// AUTHOR
...
public class DList<E> 
{
     ...
     * currently at that index (if it exists) and succeeding elements to the right (i.e., adds one
     * to their indices).
     *
     * Note that if pIndex == getSize() then new element is appended to the list.
     *
     ...
     * add(int pIndex, E pData) will cause add() to perform an append operation.
     */
    ...
    protected static class Node<E> {
         ...
         *
         * 1. this Node cannot be null because if it were we would not have been able to call the
         *    equals() method on it. However, pNode might be null and if it is then clearly this
         *    Node and pNode are not equal so we return false.
         * 2. If the address of this Node and pNode are the same then they are the same object
         *    so they are clearly equal to each other so we return true.
         * 3. If this Node and pNode are objects of different classes then they cannot be equal
         *    so we return false.
         * 4. If the instance variables of this Node are equal to the instance variables of pNode
         *    then the two objects are considered equal so return true.
         * 5. Otherwise, returns false.
         *
         * Also, the parameter object to equals() has to be of type Object because we are over-
         * riding the equals() method declared in Object and for Object.equals() the paramemter
         * is an Object. However, we need to call the getData(), getNext(), and getPrev() methods
         * on pNode and we cannot do that if the JVM thinks that pNode is an Object. Consequently
         * we must type case pNode to be a Node<E>. Unfortunately, if you compile with the -Xlint
         * option you will get an "unchecked cast" warning on the line where we declare node and
         * type cast pNode to Node<E>. For reasons I have not taken the time to fully understand
         * it is difficult to make this warning go away so a quick and easy and sleazy way to
         * eliminate the warning is to provide a @SuppressWarnings("unchecked") attribute which
         * tells the compiler not to complain about this uncheck cast.
         *
       ...
       /**
        ...
        public String toString() {
            return "" + getData();
        } // Ends the definition of toString()
    } // Ends the declaration of the nested Node class
} // Ends the declaration of the DList class
